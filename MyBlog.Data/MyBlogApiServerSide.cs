    ...
    public class MyBlogApiServerSide : IMyBlogApi
    {
        ...
        public async Task<BlogPost> GetBlogPostAsync(int id)
        {
            ...
            return await context.BlogPosts.Include
                (p => p.Category).Include(p => p.Tags).FirstOrDefaultAsync(p => p.Id == id);
        }
        ...
        private async Task<IMyBlogItem> SaveItem(IMyBlogItem
            item)
        {
            using var context = factory.CreateDbContext();
            if (item.Id == 0)
        ...
        public async Task<BlogPost> SaveBlogPostAsync(BlogPost
            item)
        {
